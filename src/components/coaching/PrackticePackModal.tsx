...
const prackticepackSchema = z.object({
...
// Component
const PrackticePackModal: React.FC<FormBannerProps> = ({ heading }) => {
  ...
  const courseForm = useForm<z.infer<typeof prackticepackSchema>>({
    resolver: zodResolver(prackticepackSchema),
    defaultValues: {
      packagedate: undefined,
    },
  });

  // Course info Form Hook
  ...
  const handlePrackticeSubmit = async (
    data: z.infer<typeof prackticepackSchema>
  ) => {
    console.log("Course Submitted:", data);
    setIsCourseinfoOpen(true); // ✅ just go to the next modal
  };

  const handlePrackticeinfoSubmit = async (
    ...
    <div className="enquiryform-container">
      ...
      <Dialog open={isCourseOpen} onOpenChange={handleCloseModals}>
        <DialogContent className="common-modal-form w-full max-w-xl top-[5%] translate-y-0" aria-describedby={undefined}>
          ...
          <Form {...courseForm}>
            <form
              onSubmit={courseForm.handleSubmit(handlePrackticeSubmit)}
      ...
      <Dialog open={isCourseinfoOpen} onOpenChange={handleCloseModals}>
        <DialogContent className="common-modal-form w-full max-w-xl top-[5%] translate-y-0" aria-describedby={undefined}>
          ...
          <Form {...courseinfoForm}>
            <form
              onSubmit={courseinfoForm.handleSubmit(handlePrackticeinfoSubmit)}
              className="space-y-4 p-0 w-full"
            >
...
export default PrackticePackModal;
